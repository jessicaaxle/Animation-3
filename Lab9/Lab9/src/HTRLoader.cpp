	...
	if (file)
	{
		...
		while (fgets(buffer, MAX_LINE_LENGTH, file) != NULL)
		{
			...
			if (loc)
			{
				...
				if (!processBasePositionSection(file, loc)) // DONE
					break;

				if (!processAnimationSection(file, loc)) // DONE
					break;
			}
		}
...
Transform* HTRLoader::getGameObjectByName(std::string jointName)
{
...
void HTRLoader::createGameObjects()
{
	...
	{
		SkinnedGameObject* newJoint = new SkinnedGameObject();
	...
	// Now that we have Transforms for each joint, we need to set up
	// the kinematic linkage

	// Set up the hierarchy 
	for (int i = 0; i < jointDescriptors.size(); i++)
	{
		//Shortcut tool to avoid typing a lot :P
		JointDescriptor* jd = &jointDescriptors[i];
		
		// Hint: use the getGameObjectByName function
		Transform* child  = getGameObjectByName(jd->jointName);
		Transform* parent = getGameObjectByName(jd->jointParent);
...
Transform* HTRLoader::getRootGameObject()
{
...
std::vector<SkinnedGameObject*> HTRLoader::getGameObjects()
{
...
bool HTRLoader::processAnimationSection(FILE* fp, char* loc)
{
	...
	for (int i = 0; i < jointDescriptors.size(); i++)
	{
		...
		do
		{
			...
			//%d is a decimal integer... okay?
			sscanf(buffer, "%d %f %f %f %f %f %f %f", &frame, &t.x, &t.y, &t.z, &r.x, &r.y, &r.z, &scale);

			t *= unitsScaleFactor;
			glm::quat rotation = createQuaternion(r.x, r.y, r.z);

			jointDescriptors[i].jointPositions.push_back(t);
			jointDescriptors[i].jointRotations.push_back(rotation);
